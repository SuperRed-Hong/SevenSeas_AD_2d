...
- [2026-09-11 UI 编辑约定] 用户明确要求需手调的触屏 UI 直接保存在 Hierarchy，不使用运行时代码生成。当前已将三个触屏按钮序列化在 FishingLoopTest 场景，ReelingButtonHUD 仅负责显隐和释放输入；图片、文字、字号、位置、尺寸均由场景资产控制。此前惰性创建方案被本次场景对象方案替代，先前运行验收不能当作此版本验收。
