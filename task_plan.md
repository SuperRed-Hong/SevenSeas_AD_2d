...
## 2026-09-09 — FishingLoopTest检修

- [完成] 撤回误改Level 1/2/3；静态核对正式场景关键组件、Profile、Layer、启用状态，未发现断线。
- [编译完成/待采集] Unity只读接线报告及当前故障运行日志；鱼行为根因与真机震动仍未确认，不视为修复完成。

## 2026-09-09 — UI Select 悬停音效排查

- [素材引用已调整/待试听] UI Select 改用现有单次短音效。Unity 验证鼠标进入后停住、在按钮内移动、移出重入；若仍连响，继续采集 PointerEnter/Select 事件次数。

## 2026-09-09 — UI hover 复现补充与修复（更正）

- [代码/编译/隔离检查完成，正式试听待验收] UI hover 按指针去重，原音效已恢复。Unity 中复测同一按钮背景与文字间移动保持静音、移出重入响一次、点击确认及键盘导航。前条素材替换方案已撤回。

## 2026-09-09 — 地图与开场镜头交接（最新）

- [实现/专项验证完成，正式试玩待验收] 三图随机初始化与空mapContainer恢复、独立鱼矩形、跨生成器防重叠、鱼钩和鱼统一Tilemap障碍识别。详见地图/鱼范围参考记录；不能用隔离测试代替当前场景验收。
- [用户已确定流程，尚未实施] 开场与随机地图无关的空镜 → Start后缓慢移到地图上方 → 电影画幅由上向下Overview Roll → ShorePlayer平台 → 既有游戏镜头。画幅、时长、跳过、教程/校准编排待下一chat讨论。
- [交接完成] docs/reference/2026-09-09-map-intro-camera-handoff.md包含现状、接口、已定规则、待决项和验收。下一chat负责镜头；原chat继续Editor工具教学。
- [教学下一步] 用户已写GetSelectedSprites，OnGUI尚未调用；改为List返回值并将Length改Count，再验证外层Texture2D读取/去重，不代写。
- [Git状态] 三文档工作区已追加本记录，索引仍UU；本轮未暂存/提交/推送。

## 2026-09-09 — 独立菜单地图与开场巡览实施

- [源码/场景/原生专项验证完成] 用户确认两块独立场地：本局三图等概率，菜单从剩余两图抽取，仅地图/水面/游动鱼，无人物平台。新增运行时菜单布景、2秒入场/5秒下巡/0.6秒平台停留/0.8秒交回、10%上下黑边及SKIP；教程和校准在前，每局播放，完成后才开启玩法。
- [通过] 临时副本当前正式场景七次加载103断言；补充零时长和暂停时非缩放演出通过。1440×2304截图检查通过；源码/场景/Profile SHA256与当前工作区一致。
- [待验收] 用户当前编辑器手感、Android/WebGL、实际校准、Play Again/排行榜完整回归。720×1152原菜单布局裁切另记，未重排菜单。
- 调参和证据：docs/reference/2026-09-09-map-intro-camera-implementation.md。未改障碍Prefab教学文件，未提交/推送。当前Git索引已非旧交接的UU状态，本轮未处理合并或暂存。

- [2026-09-09 更正] 用户要求下巡本身8秒，实际资产/default已由5改8；总演出约11.75秒。本次仅静态确认参数及读取链。菜单独立装饰工作流待明确是否需要与游戏地图分开。

- [2026-09-09 最新更正] 下巡改Survey Speed=2世界单位/秒；新增独立IntroCamera；修复EntryGuard.intro空引用绕过开场，原生空引用注入/黑边/速度专项通过。当前Editor与设备试玩仍待用户验收。

## 2026-09-10 — 镜头验收与移动端菜单修复

- [用户验收完成] 用户明确确认地图/开场镜头这部分完成；不等同于整个Prototype结束或所有平台验收。
- [源码/场景/原生专项通过] Start透明图片命中范围收紧，触屏落指Select音取消，只在有效点击后播放确认音。640×480与1440×2304各13项Input System触摸检查通过。
- [待设备复测] 实机空白处不误触Start、按钮落指静音/抬指一声、拖出取消；见docs/reference/2026-09-10-mobile-menu-input-fix.md。

## 2026-09-10 — GameOver 音效

- [源码/场景/音源专项通过] 独立Game Over Clip/Level接入，复用现有失败素材；结束一次播放、最后一钩避免叠音，原生专项9项通过。
- [待试玩] 完整计时结束/耗尽鱼钩两条流程以及Android/WebGL实际听感；需要不同结束音时替换Game Over Clip。

## 2026-09-10 — 结束展示与设置系列

- [用户确认/源码场景完成/原生专项通过] 3秒GameOver→Inventory→1秒出现两种去向，不再自动跳排行榜；18项专项通过。
- [已实现/竖屏截图检查] Settings、Developer、校准、暂停、调参统一配色与像素按钮；保存七人名单。
- [待设备验收] 手机触摸与安全区、完整游戏结束后查看鱼获→Leaderboard/Play Again加载回归；教程原动画与独立输入测试场景未重做。
- [2026-09-10] Developer Scene预览同步双行；名单前移建议已讨论，尚未实施排序。
- [2026-09-10 已完成] 用户批准名单排序，Shuo Hong (Flynn)置首，编辑预览与运行数据同步。
- [已实现 2026-09-10] Developer教程开关隐藏为右下角小透明热区，移除可见标签及提示。
- [2026-09-10 最新顺序] Production、Programming、Game Art、Technical Art、Game Design（设计保留在末尾）；七人均保留。
- [2026-09-10 最新教程入口] 用户强调游戏内可用且低调：改为Developer右下角、Close右侧的小号Tutorial文字，无背景/状态/说明；取代全透明无文字热区。
- [2026-09-10 最新Developer方案] 深蓝/白/青蓝；教程开关移到DEVELOPER标题，去掉计数和角落标签，替代小号Tutorial按钮。
